...
const agents: Agent[] = [
  ...
  },
  {
    ...
    description: "Reviews pull requests for code quality, checks against the brief for relevance, and leaves comments.",
    status: "active",
    type: "system",
  ...
  return (
    <div className="p-5 rounded-xl border border-border bg-white hover:shadow-sm transition-shadow group">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
            agent.type === "system" ? "bg-primary text-white" : "bg-accent-light border border-orange-200"
          }`}>
            <Bot className={`w-5 h-5 ${agent.type === "system" ? "text-white" : "text-accent"}`} />
          </div>
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <span className="text-[10px] text-muted-foreground">
                {agent.type === "system" ? "System" : "Custom"}
              </span>
              <span className="text-[10px] text-muted-foreground">·</span>
              <span className="text-[10px] text-muted-foreground">
                {agent.authority === "full" ? "Full control" : "Observe only"}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1"><Zap className="w-3 h-3" /> {agent.trigger}</span>
          <span className="flex items-center gap-1"><Play className="w-3 h-3" /> {agent.runs} runs</span>
        </div>
        {agent.lastRun && (
        ...
        {step === "describe" && (
          <div className="p-6">
            <p className="text-sm text-muted-foreground mb-4">
              Describe what you want this agent to do in plain text. Neaven will build the workflow for you.
            </p>
            <textarea
              value={description}
          ...
          <div className="p-6">
            ...
            <div className="rounded-lg bg-accent-light border border-orange-200 p-3 mb-6">
              <p className="text-xs text-accent">
                ...
                This agent needs: <strong>Vercel</strong> and <strong>Slack</strong> connectors.
                {" "}<a href="/connectors" className="underline">Connect them →</a>
              </p>
            </div>

            <div className="mb-6">
              <label className="block text-sm font-medium mb-1.5">Co-founder authority over this agent</label>
              ...
              <p className="text-xs text-muted-foreground mt-1">Controls how much your AI co-founder can influence this agent.</p>
            </div>
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Create agents that work for you 24/7. Describe what you want in plain text — tweak the details if you want to.
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="p-4 rounded-xl border border-border bg-white">
          <p className="text-2xl font-bold">{agents.reduce((acc, a) => acc + a.runs, 0)}</p>
      ...
      <div className="flex items-center gap-1 mb-6 border-b border-border">
        {[
          ...
          { id: "paused" as const, label: "Paused", count: agents.filter(a => a.status === "paused").length },
          { id: "draft" as const, label: "Drafts", count: agents.filter(a => a.status === "draft").length },
        ].map((tab) => (
          <button
            key={tab.id}
