...
- **Activepieces-cloud independence** (audited 2026-07-17): `AP_TELEMETRY_ENABLED=false` (also gates template-usage telemetry) and `AP_CLOUD_AUTH_ENABLED=false` (no shared OAuth apps — previously app connections OAuth'd through `secrets.activepieces.com` under Activepieces branding, and token refresh kept flowing through them; connections created before the switch must be deleted and recreated). What still calls out: piece **catalog metadata** sync from `cloud.activepieces.com` (`PIECES_SYNC_MODE=OFFICIAL_AUTO`) and the templates catalog when a user browses templates — read-only catalog fetches, no user data leaves the box. Piece packages install from the public npm registry. With cloud auth off, each OAuth-based connector needs our own OAuth client (e.g. a "Neaven" Google Cloud OAuth app for Gmail; redirect URI `https://automations.neaven.net/redirect`).
- **Engine sandbox networking gotcha:** the flow-execution sandbox resolves its API callback from `AP_FRONTEND_URL`, so that URL must be reachable *from inside the container*. Locally the app therefore listens on 8080 in-container (`AP_PORT=8080`, mapped `8080:8080`) so `http://localhost:8080` is valid on both sides — with the default port-80 listener every test/publish failed with `fetch failed`/`ECONNREFUSED`. In production the public URL resolves fine from inside once DNS exists.
